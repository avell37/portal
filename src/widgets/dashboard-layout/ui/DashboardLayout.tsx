...
import { BookOpen, GraduationCap, LayoutDashboard, Monitor, Users } from 'lucide-react'
import { useAuth } from '@/entities/session'
import { AppTopbar } from '@/widgets/app-topbar'
import logo from '@/shared/assets/images/logo.png'

const NAV_ITEMS = [
  { path: '/director', label: 'Дашборд директора', icon: LayoutDashboard, roles: ['director'] },
  { path: '/uchebny', label: 'Учебный отдел', icon: BookOpen, roles: ['director', 'uchebny_head'] },
  { path: '/teacher-analytics', label: 'Преподаватели', icon: GraduationCap, roles: ['director', 'uchebny_head', 'teamlead'] },
  { path: '/vospitatelniy', label: 'Воспитательный отдел', icon: Users, roles: ['director', 'vospitatelny_head', 'curator'] },
  { path: '/it-support', label: 'IT-поддержка', icon: Monitor, roles: ['director', 'it_admin'] },
]
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-[14px] px-3 py-2.5 text-[14px] font-medium transition-colors ${
                    isActive ? 'bg-auth-primary text-white' : 'text-auth-black hover:bg-auth-bg'
                  }`
                }
              >
                {({ isActive }) => (
